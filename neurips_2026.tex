...
\end{align}
%
where $\e_\theta(\z_t, t):=\x_\theta(\z_t, t) \Emb$ is the predicted clean embedding, and $\langle\cdot,\cdot\rangle$ denotes the inner product. Crucially, the NELBO depends on $\Emb$, which justifies jointly learning $\Emb$ and the denoising model to optimize likelihood. During training, Plaid adaptively allocates a fraction of each batch to the reconstruction loss (Monte Carlo over $\z_0$) and the remainder to the diffusion loss (Monte Carlo over $t$ and $\z_t$); the prior loss uses the whole batch. See~\cref{supp:sec:training-algo} for the training algorithm.
...
\begin{figure*}[t]
    \centering
    \begin{subfigure}{0.3\linewidth}\phantomsubcaption\label{fig:sample-quality}\end{subfigure}%
    \begin{subfigure}{0.3\linewidth}\phantomsubcaption\label{fig:mauve_vs_T}\end{subfigure}\\
    \begin{subfigure}{0.3\linewidth}\phantomsubcaption\label{fig:genppl_vs_entropy}\end{subfigure}%
    \includegraphics[width=1.0\linewidth]{figures/plaid_3panel_elf.pdf}
    \caption{\small \textbf{RePlaid achieves superior unconditional generation quality.} We sample from 0.1B models trained for 1M on OWT ($L = 1024$,
$N = 5120$, no temperature). \textbf{(a-b)} GenPPL and MAUVE. GenPPL does not capture diversity while MAUVE does as a distributional metric. \textbf{(c)} GenPPL-entropy frontier as $T$ increases. We observe that Duo, LangFlow, ELF, and FLM have worse entropy than RePlaid at comparable GenPPL levels.}
...
FLM \zhihan{should we keep this? maybe mention only in sampling, together with ELF}

\paragraph{Setup.} We apply these four samplers to sample unconditionally ($L=1024$) from RePlaid (s.c.) and (no s.c.) trained on OWT for 1M steps. No temperature is used. During sampling, we vary the number of diffusion discretization steps $T$ (not including the final $\argmax$) to control NFEs. For MDLM, Duo, FLM, and LangFlow, we use their official, publicly available 1M checkpoints. We measure Generative perplexity (GenPPL)~\citep{dieleman2022continuous} via GPT-2 Large~\citep{radford2019language}, MAUVE~\citep{pillutla2021mauve} via ModernBERT-Large~\citep{warner2025smarter} \zhihan{we release this package} for sample quality, and average unigram entropy for diversity~\citep{zheng2024masked}. GenPPL is the standard metric used in prior work~\citep{sahoo2024simple, sahoo2025the, lee2026flow, potaptchik2026discrete, chen2026langflow}. Unlike prior work, we also measure MAUVE, a distributional metric designed to capture both quality and diversity that has been shown to correlate with human judgements~\citep{pillutla2021mauve}.
...
\begin{figure*}[t]
    ...
    \begin{subfigure}{0.4\linewidth}
        ...
        \caption{Self-conditioning \zhihan{update self cond numbers, and elf}}
...
\begin{table}[h]
  \centering
  \small
  \caption{\small \textbf{Mathematical reasoning}. We train 0.1B models for 250K steps on TinyGSM~\citep{liu2023tinygsm} and prompt them using the GSM8K~\citep{cobbe2021gsm8k} test set. We use \texttt{pass@1} and $\text{max response length}=512 - \text{prompt length}$. The AR, MDLM, Duo, and FLM numbers are obtained from the official checkpoints of concurrent work \citet{deschenaux2026language}. The code responses are evaluated by a Python interpreter. Diffusion uses ancestral sampling with $T=1024$. Logit temperature: dividing the logits of $\x_\theta(\z_t, t)$ by $\tau$ before the softmax; score temperature: rescaling the predicted noise $\beps_\theta(\z_t, t)$ (equivalently the diffusion score) by $1/\tau$, following~\citet{dieleman2022continuous}. For both, $\tau<1$ sharpens. ``s.c.'' indicates self-conditioning (used during both training and evaluation). ``N.A.'' marks entries that are not applicable (methods without a score interface, or sampler configurations we do not evaluate). The five best values across all methods (excluding AR) are \textbf{shaded}, from darkest (best) to lightest.}
  \vspace{0.5em}
  \label{tab:tinygsm}
   \scriptsize
  \begin{tabular}{llccccccc}
    \toprule
    & & & \multicolumn{3}{c}{Ancestral Sampler ($T=1024$)} & \multicolumn{3}{c}{Adaptive-Order Samplers} \\
    \cmidrule(lr){4-6} \cmidrule(lr){7-9}
    & Method & s.c. & No Temp. & Best $\tau_\text{logit}$ & Best $\tau_\text{score}$ & Conf. Thres. & Top Margin & Ent. Bound \\
    \midrule
    \multicolumn{3}{l}{\textit{Autoregressive}} \\
    & Transformer & & $52.9$ & $64.4$ ($0.2$) & N.A. & N.A. & N.A. \\
    \midrule
    \multicolumn{3}{l}{\textit{Discrete Diffusion}} \\
    & MDLM~\citep{sahoo2024simple} & & $17.0$ & $35.6$ ($0.1$) & N.A. & $37.0$ ($0.8$) & \rankthree{$\mathbf{40.3}$} & $23.0$ ($0.0$) \\
    & Duo~\citep{sahoo2025the} & & $18.6$ & $37.2$ ($0.1$) & N.A. & N.A. & N.A. \\
    \midrule
    \multicolumn{3}{l}{\textit{Continuous Diffusion}} \\
    & FLM~\citep{lee2026flow} & & $0.3$ & -- & -- & N.A. & N.A. \\
    & \textbf{\textsc{RePlaid} (Ours)}, DDPM & \ccross & $13.3$ & $20.0$ ($0.05$) & \rankfive{$40.0$} ($0.05$) & N.A. & N.A. \\
    & \textbf{\textsc{RePlaid} (Ours)}, DDIM & \ccross & $9.6$ & $16.8$ ($0.5$) & \rankfour{$40.1$} ($0.01$) & N.A. & N.A. \\
    & \textbf{\textsc{RePlaid} (Ours)}, DDPM & \cmark & $33.2$ & $38.8$ ($0.0001$) & \rankone{$\mathbf{49.0}$} ($0.01$) & N.A. & N.A. \\
    & \textbf{\textsc{RePlaid} (Ours)}, DDIM & \cmark & $26.8$ & $31.2$ ($0.3$) & \ranktwo{$48.4$} ($0.01$) & N.A. & N.A. \\
    \bottomrule
  \end{tabular}
  \vspace{-0.5em}
\end{table}
